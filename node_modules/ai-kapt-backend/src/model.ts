...
import { Card, GameSnapshot, Move } from "./types.js";
...
export function encodeState(snapshot: GameSnapshot, playerIndex: number): number[] {
  const player = snapshot.players[playerIndex];
  const ownHand = encodeCardCount((player?.hand ?? []) as Card[]);
  const tableAttacks = encodeCardCount(snapshot.table.map((entry) => entry.attack));
  const tableDefenses = encodeCardCount(snapshot.table.map((entry) => entry.defense).filter((card): card is Card => Boolean(card)));
  const playerCounts = snapshot.players.map((entry) => entry.handCount / 20).slice(0, 4);
  while (playerCounts.length < 4) playerCounts.push(0);
...
export function actionIndexToMove(actionIndex: number, legalMoves: Move[]): Move {
...
export function moveToActionIndex(move: Move): number {
  ...
  if (move.type === "pass") return 54;
  if (move.type === "take") return 55;
  return 57;
}

function cardActionToIndex(cardId: string): number {
  if (cardId === "joker-black") return 52;
  if (cardId === "joker-red") return 53;
  const match = /^(spades|clubs|hearts|diamonds)-(\d+)$/.exec(cardId);
  if (!match) return 57;
  const suitOffset = { spades: 0, clubs: 13, hearts: 26, diamonds: 39 }[match[1] as "spades" | "clubs" | "hearts" | "diamonds"];
...
export class DurakPolicy {
  ...
  async choose(snapshot: GameSnapshot, playerIndex: number, legalMoves: Move[], epsilon = 0.1): Promise<PolicyDecision> {
  ...
  async load(modelPath: string): Promise<boolean> {
    try {
      ...
      const data = JSON.parse(raw) as {
        inputSize: number;
        weights: Array<{ shape: number[]; values: unknown }>;
      };
      if (data.inputSize !== this.inputSize) return false;
      const tensors = data.weights.map((entry) => tf.tensor(entry.values as tf.TensorLike, entry.shape));
